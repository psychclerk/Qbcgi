...
class ReturnSignal(Exception):
    """Internal control-flow for returning from user-defined functions/subs."""
...
class Parser:
    ...
    def _parse_block(self, start: int, terminators: set[str]) -> tuple[list[Any], int]:
        ...
        while i < len(self.lines):
            ...
            if upper.startswith("SUB "):
                name_and_params = raw[4:].strip()
                name, params = self._parse_callable_signature(name_and_params)
                sub_body, end_sub = self._parse_block(i, {"ENDSUB", "END SUB"})
                i = end_sub + 1
                body.append(Block("sub", (name, params), sub_body))
                continue

            if upper.startswith("FUNCTION "):
                name_and_params = raw[9:].strip()
                name, params = self._parse_callable_signature(name_and_params)
                fn_body, end_fn = self._parse_block(i, {"ENDFUNCTION", "END FUNCTION"})
                i = end_fn + 1
                body.append(Block("function", (name, params), fn_body))
                continue
    ...
    def _parse_callable_signature(self, text: str) -> tuple[str, list[str]]:
        if "(" not in text:
            return text.strip(), []
        if not text.endswith(")"):
            raise QBError(f"Invalid callable signature: {text}")
        name, arglist = text[:-1].split("(", 1)
        params = [p.strip() for p in arglist.split(",") if p.strip()]
...
class Interpreter:
    def __init__(self, program: list[Any], ctx: ExecContext):
        ...
        self.user_subs: dict[str, tuple[list[str], list[Any]]] = {}
        self.user_functions: dict[str, tuple[list[str], list[Any]]] = {}
        self._extract_callables(program)
        for fn_name in self.user_functions:
            self.eval.functions[fn_name.upper()] = self._make_function_wrapper(fn_name)

    def run(self) -> None:
        self._exec_block(self.program)

    def _extract_callables(self, statements: list[Any]) -> None:
        for stmt in statements:
            if isinstance(stmt, Block) and stmt.kind == "sub":
                name, params = stmt.header
                self.user_subs[name.upper()] = (params, stmt.body)
            elif isinstance(stmt, Block) and stmt.kind == "function":
                name, params = stmt.header
                self.user_functions[name.upper()] = (params, stmt.body)
            else:
                self.program.append(stmt)

    def _make_function_wrapper(self, name: str) -> Callable[..., Any]:
        upper = name.upper()

        def wrapper(*args: Any) -> Any:
            return self._invoke_callable(upper, list(args), is_function=True)

        return wrapper

    def _invoke_callable(self, name: str, args: list[Any], *, is_function: bool) -> Any:
        table = self.user_functions if is_function else self.user_subs
        target = table.get(name.upper())
        if not target:
            kind = "Function" if is_function else "Sub"
            raise QBError(f"{kind} not found: {name}")
        params, body = target
        if len(params) != len(args):
            raise QBError(f"{name} expects {len(params)} args, got {len(args)}")

        backup: dict[str, tuple[bool, Any]] = {}
        for pname, pval in zip(params, args):
            exists = pname in self.ctx.vars
            backup[pname] = (exists, self.ctx.vars.get(pname))
        ...
        try:
            ...
            for pname, (exists, oldval) in backup.items():
                if exists:
                    self.ctx.vars[pname] = oldval
                else:
                    self.ctx.vars.pop(pname, None)
    ...
    def _exec_stmt(self, stmt: str) -> None:
        ...
        if upper.startswith("CALL "):
            payload = stmt[5:].strip()
            if "(" in payload and payload.endswith(")"):
                name, arglist = payload[:-1].split("(", 1)
                args = self._split_args(arglist)
                values = [self.eval.eval(a) for a in args]
            else:
                name = payload
                values = []
            self._invoke_callable(name.strip().upper(), values, is_function=False)
            return
